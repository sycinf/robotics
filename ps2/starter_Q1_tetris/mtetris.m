...
global MODE TIMESTEP MAP POS GEO BOXOFF BOXY SNDFLAG SNDS SNDD SNDROW SHAPEIND board_data;
...
if nargin ~= 1 | cmd == 'C'
   ...
   SHAPEIND = ceil(rand(1)*size(Shapes,3));
    ...
    for translation = 1:10
        for rotation = 0:3
            map1 = tetris_place_block(MAP, SHAPEIND, translation, rotation, board_data);
...
while ~MODE		% main game loop   
   ...
   if cmd == 'K' & ~MODE
      ...
      case {'5','w','W'}		% spin shape
         if ~any((BOXOFF*POS + [-1,BOXY]*GEO - BOXY) > length(MAP(:))) & ~any((BOXOFF*POS + [-1,BOXY]*GEO - BOXY) < 1 ) & ~any(MAP(BOXOFF*POS + [-1,BOXY]*GEO - BOXY))
