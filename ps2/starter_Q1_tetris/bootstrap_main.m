...
stored_result_ind = 1;
while(cur_avg >= prev_avg)

    prev_avg = cur_avg
    % now re create all the map things
    if(prev_avg != 0)
        clearvars backup_maps 
        %make new backup_maps
        k = 1;
        for b=1:length(chosen_map_log)
            if(mod(b,subsample_interval)==0)
                backup_maps{k} = chosen_map_log{b};
                ...
                for rotation = 0:3
                    for translation = 1:10
                        ...
                        if(game_over)
                            ...
                            R{b}{block_idx}(i,1) = tetris_reward(map1); 
                        end
                        i = i+1;
                    end
                end
    ...
    cvx_begin
        ...
        variable s(numSamples)
        minimize ((mu* (the.' * backup_states_phi).'))
        subject to
        for b=1:numSamples
            numBlock*the.'*backup_states_phi(:,b) >= sum(I(b,:))-s(b);
            s(b)>=0;
            for k=1:numBlock
                i = 1;
                for rotation = 0:3
        ...
        1/numSamples*sum(s) <= slack_amount;
        theta=the;
    cvx_end
    ...
    board_data = init_board_data();
    mapind = 1;
    clearvars chosen_map_log
    for i=1:20
        [scores(i), all_scores{i}, avg_num_blocks_placed(i),cur_map_log] = main_evaluate_theta(theta, feature_f, i+10, board_data);
        length(cur_map_log)
        for cur_map_log_ind = 1:length(cur_map_log)
            chosen_map_log{mapind} = cur_map_log{cur_map_log_ind};
            mapind = mapind+1;
        end
    end
    avg_num_blocks_placed

    cur_avg= mean(avg_num_blocks_placed)
    stderr = std(avg_num_blocks_placed)/sqrt(length(avg_num_blocks_placed))
    resultStore(stored_result_ind,1) = cur_avg;
    resultStore(stored_result_ind,2) = stderr;
